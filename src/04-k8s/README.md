...
当前默认版本已回退到 `Cilium 1.17.1`。

这样处理的原因是：现有这套 `sealos run kubernetes-docker helm cilium` 方案在 `labring/cilium:1.18.1` 上，实际安装时仍然可能触发 `kubeProxyReplacement must be explicitly set`，而外层透传 `ExtraValues` / `HELM_OPTS` 也不稳定。

因此现在的默认策略是：

- 默认直接使用 `1.17.1`
- 默认安装命令不再自动附加 `ExtraValues` / `HELM_OPTS`
- 如果你后续要继续验证 `1.18.x`，建议显式覆盖 `--cni-version` 后单独验证
